...
* @date:	2015/02/24
* @title	progress_report_benchmark
...
* @examples
*		SELECT * FROM progress_report_benchmark(101);

		SELECT * FROM progress_report_benchmark(101,1,TRUE);

		SELECT * FROM progress_report_benchmark(101,2,TRUE);
*
*		SELECT * FROM progress_report_benchmark(101,0,FALSE,'1900-01-01','2100-01-01');
*
* @note:        to drop/delete the function - DROP FUNCTION progress_report_benchmark(integer, integer, boolean, date, date);
*/

CREATE OR REPLACE FUNCTION progress_report_benchmark(studentId INTEGER, subjectId INTEGER DEFAULT 0, latest BOOLEAN DEFAULT FALSE, evaluationDateFrom DATE DEFAULT NULL, evaluationDateTo DATE DEFAULT NULL) 
  ...
  BEGIN
	...
	*/						 
	...
	/*
		...
		real and virtual entries that represents a certain age groups path through the task/courses whether they have been 
		completed or not. The second part comes from the temp_latest_task_evaluation which are the tasks that a student has 
		attempted and/or completed. Since there is a good chance that a student has not completed all the tasks in their task 
		path, but all the task path entries will need to be seen to compare the student's task path with the student's evaluations
	...
	*/			
	...
	IF (evaluationDateFrom IS NOT NULL) AND (evaluationDateTo IS NOT NULL) THEN
		...
		SELECT temp_task_path."minimumEvaluationValue",
			...
			temp_task_path."daysToComplete"::SMALLINT AS "recommededDaysToComplete" ,			
